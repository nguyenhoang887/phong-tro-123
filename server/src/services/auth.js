...
export const registerService = ({phone, password, name}) => new Promise(async(resole, reject) => {
    try {
...
export const loginService = ({phone, password}) => new Promise(async(resole, reject) => {
    try {
        ...
        resole({
            err: token ? 0 : 2,
            msg: token ? 'Login is succesfully !' : response ? 'Password is wrong !' : 'Phone number not found !',
            token: token || null
        })
    } catch (error) {
        reject(error)
